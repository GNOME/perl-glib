...
##			}
	...
	while (<IN>) {
		...
		if (is_module_line ($_)) {
			...
			push @{ $data{$package}{xsubs} }, split_aliases ($xsub);
...
foreach $package (keys %data) {
	next unless $data{$package}{pods};
	next unless $data{$package}{xsubs};
	my $pods = $data{$package}{pods};
	for (my $i = @$pods-1 ; $i >= 0 ; $i--) {
		my $firstline = $pods->[$i]{lines}[0];
		next unless $firstline =~ /=for\s+apidoc\s+([:\w]+)\s*$/;
		my $name = $1;
		foreach my $xsub (@{ $data{$package}{xsubs} }) {
...
print Data::Dumper->Dump([\@xspods, \%data],
                       [qw($xspods   $data)]);
print "\n1;\n";
